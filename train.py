...
from datasets.hellaswag.prepare import iterate_examples, render_example
...
class DataLoaderLite:
    ...
    def __init__(self, B, T, data_root, process_rank, num_processes, split):
        self.B = B
        self.T = T
...
def train(
        data_root="./datasets/fineweb/edu_fineweb10B",
        B=32, # micro batch size
        T=1024, # sequence length
        max_lr=6e-4, # min_lr will be set to max_lr * 0.1
    ...
    train_loader = DataLoaderLite(B=B, T=T, data_root=data_root, process_rank=ddp_rank, num_processes=ddp_world_size, split="train")
    val_loader = DataLoaderLite(B=B, T=T, data_root=data_root, process_rank=ddp_rank, num_processes=ddp_world_size, split="val")
    ...
    # create the log directory we will log to
    log_dir = "log"
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, f"{run_name}_logs.txt")
    with open(log_file, "w") as f: # open for writing to clear the file
        pass

    # create model directory we will write checkpoints to
    models_dir = "models"
